...
test('Minutes to Seconds', () => {
    ...
    expect(startConversion('Minutes', 'Seconds', -1, true)).toBe("Invalid input!");
});

test('Hours to Seconds', () => {
    expect(startConversion('Hours', 'Seconds', 0.3, true)).toBe(1080);
    expect(startConversion('Hours', 'Seconds', -1, true)).toBe("Invalid input!");
});

test('Days to Seconds', () => {
    expect(startConversion('Days', 'Seconds', 2, true)).toBe(172800);
    expect(startConversion('Days', 'Seconds', -1, true)).toBe("Invalid input!");
});
...
test('Seconds to Minutes', () => {
    ...
    expect(startConversion('Seconds', 'Minutes', -1, true)).toBe("Invalid input!");
});

test('Hours to Minutes', () => {
    expect(startConversion('Hours', 'Minutes', 3, true)).toBe(180);
    expect(startConversion('Hours', 'Minutes', -1, true)).toBe("Invalid input!");
});

test('Days to Minutes', () => {
    expect(startConversion('Days', 'Minutes', 2.3, true)).toBe(3312);
    expect(startConversion('Days', 'Minutes', -1, true)).toBe("Invalid input!");
});
...
test('Seconds to Hours', () => {
    ...
    expect(startConversion('Seconds', 'Hours', -1, true)).toBe("Invalid input!");
});

test('Minutes to Hours', () => {
    expect(startConversion('Minutes', 'Hours', 600, true)).toBe(10);
    expect(startConversion('Minutes', 'Hours', -1, true)).toBe("Invalid input!");
});

test('Days to Hours', () => {
    expect(startConversion('Days', 'Hours', 1.1, true)).toBe(26.4);
    expect(startConversion('Days', 'Hours', -1, true)).toBe("Invalid input!");
});
...
test('Seconds to Days', () => {
    ...
    expect(startConversion('Seconds', 'Days', -1, true)).toBe("Invalid input!");
});

test('Minutes to Days', () => {
    expect(startConversion('Minutes', 'Days', 2160, true)).toBe(1.5);
    expect(startConversion('Minutes', 'Days', -1, true)).toBe("Invalid input!");
})

test('Hours to Days', () => {
    expect(startConversion('Hours', 'Days', 120, true)).toBe(5);
    expect(startConversion('Hours', 'Days', -1, true)).toBe("Invalid input!");
});